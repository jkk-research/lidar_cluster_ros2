...
void ClusterOutline::computeOutline(
    pcl::PointCloud<pcl::PointXYZI>::Ptr& pointcloud, 
    visualization_msgs::msg::MarkerArray& hull_markers, 
    int max_added_points, int max_clust_reached, std::string frame_id) {
    ...
    for (const auto& point : pointcloud->points) {
        ...
        point_xyz.x = point.x;
        point_xyz.y = point.y;
        point_xyz.z = 0; // Set z to 0, outline height doesn't matter
    ...
    for (const auto& cluster : clusters) {
        ...
        hull_marker.color.r = 0.30; // 0.30 0.69 0.31 md_green_500
        ...
        // Add the hull marker to the array
        hull_markers.markers.push_back(hull_marker);

        // **New Code: Add a TEXT_VIEW_FACING marker for distance**
