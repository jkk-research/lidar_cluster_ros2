...
def find_clusters(points, eps, p_size=50):
    global my_cluster
    actual_cluster_id = 0
...
def update(frame):
    global my_cluster, cluster_plot, tail_plot, head_plot, points
    points = find_clusters(points, 0.8, frame + 5)
    # print(len(points), frame)
    x = [p.x for p in points]
    y = [p.y for p in points]   
    cluster_plot.set_xdata(x[:frame]) 
    cluster_plot.set_ydata(y[:frame])
